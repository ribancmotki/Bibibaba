...
function renderOutcome(outcome: SearchOutcome, repository: MarkdownRepository): string {
...
export function createMcpServer(repository: MarkdownRepository, toolName: string): Server {
          ...
          inputSchema: {
            ...
            properties: {
              query: {
                type: "string",
                description:
                  "An exact relative path of a Markdown file inside the repository to read its complete content, or free text to search across all Markdown files."
              }
            },
            required: ["query"],
